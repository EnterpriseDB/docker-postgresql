...
COPY ./RPM-GPG-KEY-PGDG-11 \
	./postgresql11-11.16-1PGDG.rhel8.x86_64.rpm \
	./postgresql11-contrib-11.16-1PGDG.rhel8.x86_64.rpm \
	./postgresql11-server-11.16-1PGDG.rhel8.x86_64.rpm \
	./postgresql11-libs-11.16-1PGDG.rhel8.x86_64.rpm \
	./pgaudit13_11-1.3.4-1.rhel8.x86_64.rpm \
	./requirements.txt \
	./*.whl \
	./*.tar.gz \
	/tmp/
...
RUN dnf update -y --nodocs && \
    dnf clean all && \
	rpm --import /tmp/RPM-GPG-KEY-PGDG-11 && \
	dnf -y reinstall \
	tar \
	glibc-common  && \
	...
	redhat-rpm-config \
	rsync \
	/tmp/postgresql11-11.16-1PGDG.rhel8.x86_64.rpm \
	/tmp/postgresql11-contrib-11.16-1PGDG.rhel8.x86_64.rpm \
	/tmp/postgresql11-server-11.16-1PGDG.rhel8.x86_64.rpm \
	/tmp/postgresql11-libs-11.16-1PGDG.rhel8.x86_64.rpm \
	/tmp/pgaudit13_11-1.3.4-1.rhel8.x86_64.rpm \
	&& rm -rf /var/cache/dnf
...
RUN mkdir /docker-entrypoint-initdb.d && \
    ...
    chmod 775 "$PGDATA" && \
    sed -ri s/"#?(listen_addresses)\s*=\s*\S+.*"/"listen_addresses = '*'"/ /usr/pgsql-11/share/postgresql.conf.sample && \
    grep -F "listen_addresses = '*'" /usr/pgsql-11/share/postgresql.conf.sample
...
#
# See https://www.postgresql.org/docs/11/server-shutdown.html for more details
# about available PostgreSQL server shutdown signals.
#
# See also https://www.postgresql.org/docs/11/server-start.html for further
...
# documentation at https://www.postgresql.org/docs/11/server-start.html notes
