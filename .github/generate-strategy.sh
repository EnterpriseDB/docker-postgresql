...
generator() {
	...
	for version in "${ubi_versions[@]}"; do
		...
		while [ "$fullVersion" != "$version" ] && [ "${fullVersion%[.-]*}" != "$fullVersion" ]; do
			...
			versionAliasesPLV8+=("$fullVersion-plv8${tagSuffix}")
		...
		if [ "${version}" -ge "15" ]; then
		  entries+=("{\"name\": \"${fullVersion} UBI${ubiRelease} PLV8\", \"ubi_version\": \"$ubiVersion\", \"platforms\": \"$platforms\", \"dir\": \"UBI/$version\", \"file\": \"UBI/$version/Dockerfile.plv8.ubi${ubiRelease}\", \"version\": \"$version\", \"flavor\": \"ubi${ubiRelease}-plv8\", \"tags\": [\"$(join "\", \"" "${versionAliases[@]}")\"], \"fullTag\": \"${fullTagMultiArch}\"}")
		fi
	done
}
